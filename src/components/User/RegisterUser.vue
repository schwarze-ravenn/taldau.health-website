<template>
    <main class="login-container">
      <form class="login-form" @submit.prevent="register">
        ...
        <p class="separator-text">или зарегистрироваться с...</p>
        ...
        signInWithPopup(getAuth, provider)
          ...
          switch (error.code) {
            case "auth/invalid-email":
              errMsg.value = "Неверный формат электронной почты";
              break;
            case "auth/user-not-found":
              errMsg.value = "Аккаунт с таким Email не найден";
              break;
            case "auth/wrong-password":
              errMsg.value = "Неверный пароль";
              break;
            case "auth/popup-closed-by-user":
              errMsg.value = "Всплывающее окно было закрыто до завершения входа";
              break;
            case "auth/cancelled-popup-request":
              errMsg.value = "Запрос был отменен";
              break;
            default:
              errMsg.value = "Произошла ошибка при попытке входа";
              break;
          }
        });
      }
